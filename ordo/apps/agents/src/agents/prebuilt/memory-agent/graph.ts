...
async function callModel(
  ...
  logger.debug('Memory Agent: Calling model');
...
async function storeMemory(
  ...
  logger.debug('Memory Agent: Storing memory');
...
function routeMessage(
  ...
  if (lastMessage.tool_calls?.length) {
    logger.debug('Memory Agent: Routing to store_memory');
    return "store_memory";
  }
  logger.debug('Memory Agent: Task complete');
  return END;
}
...
logger.success('Memory Agent initialized');
logger.info('Pattern: Conversation memory with persistent storage');
logger.info('Features: User-specific memory, context retention');
