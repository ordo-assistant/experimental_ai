/**
 * Web3 Agent
 * Comprehensive Solana DeFi operations using Solana Agent Kit
 */
...
interface Web3ToolsCache {
  tools: any[];
  model: any;
  agent: SolanaAgentKit;
}

let toolsCache: Web3ToolsCache | null = null;

/**
 * Initialize Web3 tools from Solana Agent Kit
 */
async function initializeWeb3Tools(): Promise<Web3ToolsCache> {
  if (toolsCache) {
    return toolsCache;
  }
  ...
  try {
    ...
    logger.success('Web3 Agent initialized');
    logger.info(`Wallet: ${wallet.publicKey.toString()}`);
    logger.info(`Tools loaded: ${tools.length}`);

    return toolsCache;
  } catch (error) {
    logger.failure('Failed to initialize Web3 Agent', error);
    throw error;
  }
}
...
async function callModel(state: typeof MessagesAnnotation.State) {
  const { model } = await initializeWeb3Tools();
...
async function callTools(state: typeof MessagesAnnotation.State) {
  const { tools } = await initializeWeb3Tools();
...
logger.success('Web3 Agent ready');
